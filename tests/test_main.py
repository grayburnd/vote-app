...
def docker_env():
    with DockerCompose(
        path, compose_file_name=["compose.yml"], pull=False ##Set pull to False to use local images, may need to change in GH Actions
    ) as compose:
...
def test_vote_postgres_count(docker_env: DockerCompose, vote_choice: str):
    ...
    try:
        ...
        raise requests.exceptions.HTTPError(f"failed with error code: {e.errno}\nerror: {e}")
    postgres_hostname = docker_env.get_service_host(os.environ["DB_HOST"],int(os.environ["DB_PORT"])) ##Use the docker service name and resolve the endpoint exposed to the host #0.0.0.0:5432 -> container postgres:5432
...
@pytest.mark.parametrize("target_method,side_effect,error_raised,target_func,message", 
                         [("redis.Sentinel",
                           redis.sentinel.MasterNotFoundError,
                           redis.RedisError,get_redis,
...
def test_raises(mocker: pytest_mock.MockerFixture, target_method: str, side_effect: type[Exception], error_raised: type[Exception], target_func: Callable[[], redis.Redis], message: str):
    flask_app = Flask(__name__, root_path=str(path))
    with flask_app.app_context():
        mocker.patch(
            target_method, side_effect=side_effect(message)
        )
...
def test_rpush_raises(mocker: pytest_mock.MockerFixture):
    ...
    flask_app = Flask(__name__, root_path=str(path))
    with flask_app.test_request_context("/vote", method="POST", data={"vote": "a"}):
        mocker.patch(
            "redis.Redis.rpush", side_effect=redis.exceptions.RedisError(f"There has been a Redis Error")
        )
